...
function DiseaseList (props) {
    ...
    const { loading, data, error } = useQuery(GET_DISEASE, {
        variables: {
          first: rowsPerPage,
        ...
        <Paper >
          ...
          {data && !loading && !error && (
            <Table >
              ...
              <TableBody>
                {data.diseases.map((n) => {
                  var Bloom = 'neo4j://graphapps/neo4j-bloom/?search=' + n.Name + '&perspective=FoundationBiology'
                  return (
                    <TableRow key={n.FID}>
                      <TableCell>
                        {n.FID}
                      ...
                      <TableCell component="th" scope="row" onClick={() => handleDeepLink(n.Name)}>
                        <Link to={`/diseasedetail/${n.FID_S}`} >{n.Name}</Link>
                      </TableCell>
                      <TableCell>
                      {n.AlternateNames}
                      </TableCell>
