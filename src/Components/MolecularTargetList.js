...
function MolecularTargetList (props) {
    const { classes } = props
        ...
        <Paper >
          ...
          {data && !loading && !error && (
            <Table >
              ...
              <TableBody>
                {data.molecularTargets.map((n) => {
                  var Bloom = 'neo4j://graphapps/neo4j-bloom/?perspective=FoundationBiology&search=Landscape%20for%20Target%20' + n.Name + '&run=true'
                  return (
                    <TableRow key={n.FID}>
                      <TableCell>
                        {n.FID}
...
export default MolecularTargetList;