...
function RelatedItemList() {
    ...
    const { loading, data, error } = useQuery(GET_ITEM, {
        variables: {
            ...
            substring: params.id.substring(0,params.id.indexOf("-")) ,
            subtype: params.id.substring(params.id.indexOf("-")+1,50)
        },
    })
    ...
    return (
        ...
        {data && !loading && !error && (
            ...
            <AppBar position="static">
                ...
                <TableContainer component={Paper} className={classes.container}>
                    <Table aria-label="simple table">
                        ...
                        <TableBody>
                            {data.itemsByCompanyIDAndType.map(row => (
                            <StyledTableRow key={row.FID}>
                                ...
                                row.Type === "Date" ? <Link to={`/itemdetail/${row.FID_S}`} >{row.Name}</Link> :
                                ...
                                row.Type === "ExperimentalTechnique" ? <Link to={`/itemdetail/${row.FID_S}`} >{row.Name}</Link> :
            ...
            <Typography>
              {data.itemsByCompanyIDAndType.length}
            </Typography>
        </React.Fragment>
        )}
        </div>
